...
async function main() {
  const listRes = await fetch(`${BASE}/api/norm-changes?limit=1`);
  ...
  const analyzeRes = await fetch(
    `${BASE}/api/analyze?normSourceId=${encodeURIComponent(normSourceId)}&replace=1`,
    { method: "POST" }
  );
  ...
  console.log("詳細画面:", `${BASE}/norm-changes/${detailId}`);
}
